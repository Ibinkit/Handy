...
pub const NL_DICTATION_PROMPT_SEED_VERSION: u32 = 2;
...
fn default_post_process_prompts() -> Vec<LLMPrompt> {
    vec![LLMPrompt {
        ...
        name: "Nederlands dictaat".to_string(),
        // Legacy (non-structured) mode substitutes the transcript into
        // ${output}; the Anthropic provider always uses that mode.
        prompt: r#"<transcript>
${output}
</transcript>

Hierboven staat de ruwe output van een speech-to-text-model. Jij bent een nabewerker voor spraakdictatie: geef de opgeschoonde tekst terug, klaar om direct in een applicatie geplakt te worden.
...
4. Verwijder vulwoorden en aarzelingen: eh, uhm, ehm, nou ja, zeg maar, weet je, dus ja, oké dus. Een inleidend "nou" aan het begin van een uiting hoort daar ook bij: "nou, ik denk dat we dat morgen doen" wordt "Ik denk dat we dat morgen doen." Draagt "nou" betekenis, dan blijft het staan ("doe het nou maar", "nou en of").
...
11. Is de input leeg of onverstaanbaar, geef dan een lege string terug."#
            .to_string(),
    },
    LLMPrompt {
...
mod tests {
    ...
    /// This fork ships Dutch dictation cleanup out of the box: Anthropic as
    /// provider, claude-haiku-4-5 as model, and the Dutch cleanup prompt
    /// pre-selected so the post-process hotkey works after only an API key.
    #[test]
    fn dutch_dictation_post_processing_is_default() {
        ...
        let selected = settings
            ...
            .as_deref()
            .expect("a default prompt must be selected");
        let prompt = settings
            .post_process_prompts
            .iter()
            .find(|p| p.id == selected)
            .expect("the selected default prompt must exist");

        // The Anthropic provider runs in legacy (non-structured) mode, which
        // injects the transcript through the ${output} placeholder — a prompt
